...
def load_sample_data():
    try:
        # Specify engine='xlrd' for .xls files
        landcover = pd.read_excel(LANDCOVER_URL, engine='xlrd', nrows=2000)
        population = pd.read_excel(POPULATION_URL, engine='xlrd', nrows=2000)
        realestate = pd.read_excel(REALESTATE_URL, engine='xlrd', nrows=2000)
        transport = pd.read_excel(TRANSPORT_URL, engine='xlrd', nrows=2000)
